...
7. Configure the four-entry owner menu and the `tasks: charline_tasks` rewrite shown in `OPERATIONS.md`; do not delete the underlying Hermes commands.
...
12. Open Projects/Tasks/Settings, restart Gateway and press read-only buttons on the old cards. They must reconstruct; Calendar confirmations must remain owner-bound and expire safely.
